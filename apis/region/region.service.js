...
export const addRegion = async (city) => {
  try {
    ...
    if (!cityFullName) {
      ...
      throw new NotFoundError("도시를 제대로 입력해주세요!");
    }
...
//수집 지역 삭제하는 로직
export const deleteRegion = async (city) => {
  try {
    const cityFullName = getCityFullName(city);
    ...
    const regions = await RegionModel.findOne({ regionName: cityFullName});
    ...
    //해당 지역을 데이터베이스에서 삭제하는 로직
    const deleteRegion = await RegionModel.deleteOne({regionName: cityFullName});

    if(deleteRegion.deletedCount ===0){
      logger.error(`삭제될 도시가 없습니다!`);
      throw new NotFoundError("해당 도시를 찾을 수 없습니다!");
    }

    logger.info(`${cityFullName} 도시 삭제 완료.`);
    return cityFullName;

  } catch (err) {
    logger.error(`수집 지역 도시 삭제 오류 : ${err.message}`);
    throw err;
  }
};
